...
int BTreeNode::traverse(int vnum)
{
	...
	{
		if (leaf == false) {
			int child = vnum * n + i + 1;
            cout << "_root" << vnum << ".addEdge(\"INDEX_EDGE\", _root" << child << ", \"min\", \"" << prev << "\", \"max\", \"" << keys[i] << "\")\n";
        }
        prev = keys[i];
    }
	...
	if (leaf == false) {
		int child = vnum * n + i + 1;
        cout << "_root" << vnum << ".addEdge(\"INDEX_EDGE\", _root" << child << ", \"min\", \"" << prev << "\", \"max\", \"" << "zzzzzzzzzzzzzzzz" << "\")\n";
    }
    
    {
        for (i = 0; i < n; i++) {
            ...
            for (int j=0; j<dataNodes[val].size(); j++) {
                cout<< "_data = g.V().hasLabel(\"" << label << "\").has(\"id\", " << dataNodes[val][j] << ").next()\n";
                cout << "_root" << vnum << ".addEdge(\"INDEX_DATA_EDGE\", _data, \"val\", \"" << val << "\")\n";
            }
        }
    }
    countTransaction ++;
