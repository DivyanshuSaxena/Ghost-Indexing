...
ofstream indexVert("indexVerticesx.csv");
ofstream indexEdges("indexEdgesx.csv");
ofstream indexDataEdges("indexDataEdgesx.csv");
...
int BTreeNode::traverse(int vnum)
{
    ...
    for (i = 0; i < n; i++)
	{
        ...
        if (leaf == false) {
                indexEdges << vnum << "|" << childNum << "|" << encodeDate[prev] << "|" << encodeDate[keys[i]] << endl;
        }
        prev = keys[i];
    }
	...
	if (leaf == false) {
		int childNum = vnum * n + i + 1;
        indexEdges << vnum << "|" << childNum << "|" << encodeDate[prev] << "|" << encodeDate[keys[i]] << "|leaf" << endl;
    }

    {
        for (i = 0; i < n; i++) {
            ...
            for (int j=0; j<dataNodes[val].size(); j++) {
                indexDataEdges << vnum << "|" << dataNodes[val][j] << "|" << label << "|" << encodeDate[val] << endl;
            }
        }
    }
