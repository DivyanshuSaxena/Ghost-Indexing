...
if [ "$#" -ne 11 ]; then
    echo "Usage: <script.sh> <label-of-vertex-in-index>
...
sed -i 's/{ATTRIBUTE_NAME}/'$8'/g' add_ghost_edges.cql
sed -i 's/{INDEX_NAME}/'$2'/g' add_ghost_edges.cql
sed -i 's/{INDEX_TYPE}/'$7'/g' add_ghost_edges.cql
...
$9/bin/cypher-shell -u ${10} -p ${11} < add_ghost_edges.cql
...
sed -i 's/'$2'/{INDEX_NAME}/g' add_ghost_edges.cql
sed -i 's/'$8'/{ATTRIBUTE_NAME}/g' add_ghost_edges.cql
sed -i 's/'$7'/{INDEX_TYPE}/g' add_ghost_edges.cql
