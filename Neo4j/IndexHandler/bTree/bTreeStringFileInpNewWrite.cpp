...
int BTreeNode::traverse(int vnum)
{
    ...
    for (i = 0; i < n; i++)
	{
        ...
        if (leaf == false) {
                indexEdges << vnum << "|" << childNum << "|" << prev << "|" << keys[i] << endl;
        }
        prev = keys[i];
    }
	...
	if (leaf == false) {
		int childNum = vnum * n + i + 1;
        indexEdges << vnum << "|" << childNum << "|" << prev << "|" << maxVal << endl;
    }

    {
        for (i = 0; i < n; i++) {
            ...
            for (int j=0; j<dataNodes[val].size(); j++) {
                indexDataEdges << vnum << "|" << dataNodes[val][j] << "|" << label << "|" << val << endl;
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (dataInpType == 'D') {
        ...
        minVal = "0";
        sprintf(&maxVal[0], "%lld", LLONG_MAX);
    }
    else {
