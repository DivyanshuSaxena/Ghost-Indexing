...
if [ "$#" -ne 14 ]; then
  ...
  <ldbc-snb-implementations-path> <Ghost-Indexing-Neo4j-folder-path>" >&2
  exit 1
fi
...
for dataset in ${datasets[*]}; do
  ...
  cd ${14}/queries
  ./run_all_params.sh $1 $dataset 0 $9 ${11} ${14}/../JanusGraph/GhostIndex ${13}/cypher/queries neo4j ghost2019
  ...
  cd ${14}/queries
  ./run_all_params.sh $1 $dataset 1 $9 ${11} ${14}/../JanusGraph/GhostIndex ${14}/ghost-queries neo4j ghost2019
done
